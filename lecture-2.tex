...
\begin{defn}[Universal Quantifiers] The universal quantifier is a symbol which expresses that the statement is true  ``for all '' or `` any'' $x$, for e.g., a statement such as `` $(\forall x\in U)P(x)$'' $\Longleftrightarrow$ If $x\in U$ then $P(x)$ is true.  
\end{defn}
...
\begin{defn}[Existential Quantifiers] The existential quantifier $\exists$ is a symbol which represents that there exists some element for which statement $P(x)$ is true. 
\end{defn}
