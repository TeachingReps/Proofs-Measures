...
\begin{defn} Let $A$ be a non-empty set and $\sim$ be an equivalence relation on $A$. 
	\begin{enumerate}
		...
		\item The \textbf{quotient set} of $A$ and $\sim$ is the set $\{\sim[x]: x \in A\}$ of all equivalence classes of $A$ with respect to $\sim$, %The quotient set of $A$ and $\sim$ 
...
\begin{enumerate}
	...
	\item Let $C \in \mathcal{D}$. Let $y \in C$. As before, it follows from the definition of $\Psi$ that $C = \sim[y]$. Therefore, by the definition of $\Phi$ we see that $C \in \Phi(\sim) = \mathcal{F}$. Hence, $\mathcal{D} \subseteq \mathcal{F}$. %We conclude that $\mathcal{F} = \mathcal{D}$.
	\end{enumerate} 
\end{enumerate} 	
\end{proof}
\end{document}