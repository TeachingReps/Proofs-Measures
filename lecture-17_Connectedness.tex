...
\begin{enumerate} [i)]
 \item \textit{Intermediate Value Theorem}.  If $f:[a, b] \to \Real$ is a continuous function and if $r$ is a real number between $f(a)$ and $f(b)$, then 
 there exists an element $c \in [a, b]$ such that $f(c) = r$.
 
 \item \textit{Maximum Value Theorem}. If $f:[a,b] \to \Real$ is continuous, then there exists an element $c \in [a, b]$ such that $f(x) \le f(c)$
 for every $x \in [a, b]$.
 
 \item \textit{Uniform Continuity Theorem}. If $f:[a,b] \to \Real$ is continuous, then given $epsilon > 0$, there exists $\delta > 0$ 
...
\begin{exmp}
Let $Y$ denote the subspace $[-1 0) \cup (0, 1]$ on the real line $\Real$. Each of the sets $[-1, 0)$ and $(0, 1]$
are nonempty and open in $Y$ (although not in $\Real$); therefore, they form a separation of $Y$. Alternatively, note that 
...
X \times \Real$, which maps $x \in X$ to $(x, b)$. Likewise, each vertical slice $a \times Y$ is homeomorphic with $Y$. As a result, the 
...
\begin{exmp}
Consider the Cartesian product $\Real^{\infty}$ in the box topology. We can write $\Real^{\infty}$ as the union of the 
...
If $a$ is a point of $\Real^{\infty}$, the open set
\begin{equation}
 U = (a_{1}-1, a_{1} + 1) \times (a_{1}-1, a_{1} + 1) \times \dots
 \nonumber 
\end{equation}
consists entirely of bounded sequences if $a$ is bounded, and of unbounded sequences if $a$ is unbounded. Thus, even through $\Real$ is connected, 
$\Real^{\infty}$ is clearly not connected as we have demonstrated its nontrivial separation.
\end{exmp}
